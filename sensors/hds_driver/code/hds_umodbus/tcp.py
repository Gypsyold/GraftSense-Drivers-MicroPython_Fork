...
class ModbusTCP(Modbus):
    ...
    def bind(self, local_ip: str, local_port: int = 502, max_connections: int = 10) -> None:
        ...
        if (
            ...
            raise ValueError("invalid TCP bind parameters")
...
class TCP(CommonModbusFunctions):
    ...
    def __init__(self, slave_ip: str, slave_port: int = 502, timeout: float = 5.0):
        ...
        """
        if (
            not isinstance(slave_ip, str)
            or not slave_ip
            or not isinstance(slave_port, int)
            or not 1 <= slave_port <= 65535
            or not isinstance(timeout, (int, float))
            or timeout <= 0
        ):
            raise ValueError("invalid TCP connection parameters")
    ...
    def _create_mbap_hdr(self, slave_addr: int, modbus_pdu: bytes) -> Tuple[bytes, int]:
        ...
        if not isinstance(slave_addr, int) or not 0 <= slave_addr <= 247 or not isinstance(modbus_pdu, (bytes, bytearray)) or not modbus_pdu:
            raise ValueError("invalid MBAP header parameters")
    ...
    def _send_receive(self, slave_addr: int, modbus_pdu: bytes, count: bool) -> bytes:
        ...
        if (
            ...
            or not isinstance(modbus_pdu, (bytes, bytearray))
            or not modbus_pdu
            or not isinstance(count, bool)
        ):
            raise ValueError("invalid TCP request parameters")
...
class TCPServer(object):
    ...
    def bind(self, local_ip: str, local_port: int = 502, max_connections: int = 10):
        ...
        if (
            ...
            raise ValueError("invalid TCP server bind parameters")
    ...
    def _send(self, modbus_pdu: bytes, slave_addr: int) -> None:
        ...
        if not isinstance(modbus_pdu, (bytes, bytearray)) or not modbus_pdu or not isinstance(slave_addr, int) or not 0 <= slave_addr <= 247:
            raise ValueError("invalid TCP response parameters")
        size = len(modbus_pdu)
        fmt = "B" * size
