...
class Modbus(object):
    ...
    def __init__(self, itf, addr_list: List[int]) -> None:
        ...
        """
        if itf is None or not isinstance(addr_list, (list, tuple)) or not addr_list:
            raise ValueError("itf and addr_list must be provided")
        self._itf = itf
    ...
    def _create_response(self, request: Request, reg_type: str) -> Union[List[bool], List[int]]:
        ...
        """
        if request is None or not isinstance(reg_type, str):
            raise ValueError("request and reg_type are invalid")
        data = []
        default_value = {"val": 0}
    ...
    def add_coil(
        ...
        if not isinstance(address, int) or not 0 <= address <= 0xFFFF:
            raise ValueError("address must be an integer from 0 to 65535")
    ...
    def _set_reg_in_dict(
        ...
        """
        if not isinstance(reg_type, str) or not isinstance(address, int):
            raise TypeError("reg_type must be str and address must be int")
    ...
    def _get_regs_of_dict(self, reg_type: str) -> dict_keys:
        ...
        """
        if reg_type == "":
            raise ValueError("reg_type must not be empty")
        if not isinstance(reg_type, str):
            raise TypeError("reg_type must be str")
        if not self._check_valid_register(reg_type=reg_type):
            raise KeyError("{} is not a valid register type of {}".format(reg_type, self._available_register_types))
    ...
    def _remove_changed_register(self, reg_type: str, address: int, timestamp: int) -> bool:
        ...
        Raises:
            KeyError: If reg_type is invalid.
        """
        if not isinstance(reg_type, str) or not isinstance(address, int) or not isinstance(timestamp, int):
            raise TypeError("reg_type must be str and address and timestamp must be int")
        result = False
    ...
    def setup_registers(self, registers: dict = dict(), use_default_vals: Optional[bool] = False) -> None:
        ...
        """
        if not isinstance(registers, dict) or not isinstance(use_default_vals, bool):
            raise TypeError("registers must be dict and use_default_vals must be bool")
        if len(registers):
